...
const Home = () => {
  ...
  const handleSubmit = async(e) => {
    ...
    try {
      setLoading(true);
      const tx = await contract.addRecord(form.name, parseInt(form.age), form.gender, form.bloodType, form.allergies, form.diagnosis, form.treatment);
      await tx.wait();
      setLoading(false);
    } catch (error) {
      console.log(error);
      ...
      <div className='bg-[#41506b] shadow-lg shadow-black rounded-xl w-11/12 md:w-[400px] h-7/12 p-6'>
          <form className='flex flex-col text-black' onSubmit={handleSubmit}>
            <div className='flex justify-between items-center'>
              <h1 className='font-bold text-2xl capitalize text-[#B1D4E0]'>add patient data</h1>
            </div>
